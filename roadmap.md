...
## 부록: Near-term priorities (Nov 2025)

- 입력장치: USB HID 키보드/마우스 (xHCI Interrupt IN, HID Report 파싱)
- 디스플레이: EDID 기반 최적 해상도 선택, VBlank 연동 렌더링 조정
- 스토리지: NVMe Admin/IO 큐, PRP, 기본 RW 경로; 저널링 FS 1종 도입
- 네트워크: e1000e 또는 RTL8168 1종 지원 추가, 링크/에러 복구 강화
- 보안: TLS 체인 검증 엄격 기본, 최소 상태기반 방화벽, Cap 기반 권한
- 전력/안정성: 장치 suspend/resume 훅 일관 적용, 수면 사이클 스트레스 테스트

우선순위 순서: USB HID → NVMe → e1000e/RTL8168 → TLS/방화벽 → 장치 훅/테스트